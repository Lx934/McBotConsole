 ...
 * 支持：代理池轮换 / 自动重连 / 多开隔离 / 配置容错 / 插件探测 / 字典模式(txt) / 配置持久化 / 调试命令行
 */
...
// ==================== 英文名字典（从 names.txt 读）====================
const NAMES_TXT_PATH = path.join(__dirname, 'names.txt');
...
// ==================== 插件探测（登录 + Tab 补全）====================
async function probePluginsViaTabComplete(host, port, version, timeoutMs = 30000) {
    return new Promise((resolve) => {
        ...
        const timeoutHandle = setTimeout(() => {
            finish({ success: false, plugins: [], error: '超时（服务器未响应或屏蔽了补全）' });
        }, timeoutMs);
        ...
        bot.once('end', (reason) => {
            if (!resolved) {
                finish({ success: false, plugins: [], error: '断开连接: ' + reason });
            }
        });
        ...
        bot.once('login', async () => {
            try {
                ...
                for (const prefix of prefixes) {
                    try {
                        const results = await bot.tabComplete('/' + prefix, true, false);
                ...
                const list = Array.from(plugins).sort();
                finish({ success: true, plugins: list, cmdCount: totalCmds });
            } catch (e) {
                finish({ success: false, plugins: [], error: e.message });
            }
        });
...
console.log = (...args) => {
    emitLog('INFO', args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' '));
};
console.error = (...args) => {
    emitLog('ERROR', args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' '));
};
...
const DEFAULT_CONFIG = {
    ...
    proxy_list: [],
    debug: false,
    log_file: `logs/bot_${INSTANCE_ID}.log`,
    json_log: `success_log_${INSTANCE_ID}.json`,
...
function createLogger(logFile, maxSizeMB, archiveDir, allowArchive = true) {
    ...
    const writeLine = (level, msg) => {
        const line = `[${new Date().toISOString().replace('T', ' ').slice(0, -1)}] [${level}] ${msg}\n`;
        checkAndRotate();
    ...
    return {
        ...
        close: () => {
            const waitDrain = () => {
                if (writeQueue.length > 0) setTimeout(waitDrain, 50);
                else if (stream) { stream.end(); stream = null; }
            };
            waitDrain();
        }
    };
...
class PluginManager {
    constructor(config, logger) {
        this.pluginsDir = config.plugin_dir || 'plugins';
    ...
    loadOrCreateCentralConfig() {
        const centralPath = path.join(__dirname, 'plugins.json');
    ...
    loadPlugin(jsPath, pluginName) {
        try {
            ...
            const logger = {
                info: (msg) => pluginLogger.info(`[${meta.name}] ${msg}`),
                error: (msg) => pluginLogger.error(`[${meta.name}] ${msg}`),
                debug: (msg) => { if (this.debug) pluginLogger.info(`[${meta.name}] DEBUG: ${msg}`); },
    ...
    reloadPlugin(pluginName) {
        ...
        const pluginDir = path.dirname(data.logFile);
        const jsPath = path.join(path.dirname(pluginDir), `${pluginName}.js`);
...
class PythonPluginManager {
    constructor(config, logger) {
        this.pluginsDir = config.plugin_dir_py || 'plugins_py';
        this.loadedPlugins = new Map();
        this.botContext = {};
        this.logger = logger;
        this.guiManager = new GuiConsoleManager(logger);
    ...
    handlePluginMessage(msg, pluginInfo) {
        if (msg.type === 'action') {
            ...
            if (level === 'error') this.logger.error(`[Python插件 ${pluginInfo.name}] ${msg.message}`);
            else this.logger.info(`[Python插件 ${pluginInfo.name}] ${msg.message}`);
        }
    }
...
class BotController {
    constructor(config) {
        ...
        ensureDir(archiveDir);
        this.logger = createLogger(config.log_file, config.log_max_size_mb || 1, archiveDir, true);
    ...
    setRuntimeConfig(key, value) {
        ...
        switch (key) {
            case 'retryDelay': this.retryDelay = num; break;
            case 'connectionInterval': this.connectionInterval = num; break;
    ...
    async start() {
        ...
        this.logger.info(`玩家名模式: ${this.config.use_name_dict ? '英文名字典' : '随机字符串'}`);
        ...
        while (this.totalAttempts < maxAttempts && !this.stopped) {
            ...
            while (attemptCount <= this.maxRetries && !success && !this.stopped) {
                ...
                if (wasSuccess) {
                    ...
                    const nonRetriable = ['Kicked', 'VersionMismatch', 'Disconnected without login', 'AntiBot'];
            ...
            if (!success) this.logger.error(`[Bot ${botId}] 最终失败，错误类型: ${lastErrorType || 'Unknown'}`);
        ...
        return new Promise((resolve) => {
            ...
            Promise.all([
                        ...
                        if (autoDisconnect > 0) {
                            disconnectTimer = setTimeout(() => {
                                if (bot && bot._client && bot._client.state !== 'closed') {
                                    userEnded = true;
                                    bot.end();
                                    this.logger.info(`[Bot ${botId}] 自动断开 (${autoDisconnect}s)`);
                                }
                            }, autoDisconnect * 1000);
                        }
                    } else {
                ...
                bot.once('kicked', (reason) => {
                    ...
                    const isAntiBot = /antibot|bot.?protect|bot.?filter|wait.*(minutes|seconds)|suspicious|too many|please wait/i.test(reasonStr);

                    let errorType = isAntiBot ? 'AntiBot'
                    ...
                    if (isAntiBot && !this.antiBotNotified) {
                        this.antiBotNotified = true;
                        this.logger.warn('⚠️ 检测到服务器启用了反机器人保护');
                        this.logger.warn('   建议：调大连接间隔 / 使用固定玩家名前缀 / 启用英文名字典模式 / 使用代理池');
                        send({
                            type: 'antibot-detected',
                            botId: botId,
                            username: playerName,
                            antiBotReason: reasonStr,
                            nameDictEnabled: this.config.use_name_dict === true,
                            proxyEnabled: this.proxyPool ? this.proxyPool.isEnabled() : false
                        });
                    }
...
async function handleCommand(input) {
    ...
    switch (cmd) {
        case 'help':
            ...
            cliWrite('  reload-names / packlog', 'white');
            cliWrite('═══════════════════════════════════════════════', 'yellow');
            break;
...
async function packLogs() {
    return new Promise((resolve) => {
        const ts = new Date().toISOString().replace(/[:\-T]/g, '').slice(0, 15);
        const packDir = path.join(__dirname, 'logs_pack');
        ensureDir(packDir);
        ...
        if (fs.existsSync(config.log_archive_dir || 'logs')) archive.directory(config.log_archive_dir || 'logs', 'main_logs');
        if (fs.existsSync(config.plugin_dir || 'plugins')) archive.directory(config.plugin_dir || 'plugins', 'node_plugins');
        if (fs.existsSync(config.plugin_dir_py || 'plugins_py')) archive.directory(config.plugin_dir_py || 'plugins_py', 'py_plugins');
...
async function handleStartFromGui(params) {
    ...
    if (controller.stopped) {
        ...
        controller.antiBotNotified = false;
