...
namespace JBSS261A
{
    public partial class SettingsDialog : Form
    {
        ...
        // ★ 玩家池
        public bool UsePlayerPool { get; set; } = false;
        public string PlayerPoolExhausted { get; set; } = "stop";
        ...
        private CheckBox chkUsePlayerPool;              // ★ 玩家池
        private ComboBox cmbPlayerPoolExhausted;        // ★ 耗尽行为
        ...
        private Panel BuildPlayerPage()
        {
            ...
            AddFullWidth(t, ref row, MakeHint("三种模式任选其一：\n  · 随机字符串：随机字母数字\n  · 英文名字典：常见英文名 + 数字\n  · 玩家池：从 player_pool.txt 按顺序取，每个用一次"));
            ...
            chkDict.CheckedChanged += (s, e) =>
            {
                this.UseNameDict = chkDict.Checked;
                if (chkDict.Checked && chkUsePlayerPool != null)
                {
                    chkUsePlayerPool.Checked = false;
                }
            };
            ...
            // ★ 玩家池
            chkUsePlayerPool = AddCheckRow(t, ref row, "", "使用玩家池（从 backend\\player_pool.txt 取名字）", UsePlayerPool);
            chkUsePlayerPool.CheckedChanged += (s, e) =>
            {
                this.UsePlayerPool = chkUsePlayerPool.Checked;
                if (chkUsePlayerPool.Checked && chkDict != null)
                {
                    chkDict.Checked = false;
                }
                if (cmbPlayerPoolExhausted != null)
                {
                    cmbPlayerPoolExhausted.Enabled = chkUsePlayerPool.Checked;
                }
            };
            ...
            AddFullWidth(t, ref row, MakeHint(
                "启用后每个名字只用一次，按文件顺序取。\n" +
                "文件不存在时程序会自动生成默认模板。\n" +
                "文件路径：backend\\player_pool.txt（每行一个名字，# 开头是注释）。"));

            // ★ 池耗尽行为
            cmbPlayerPoolExhausted = AddComboRow(t, ref row, "池耗尽后：",
                new[] { "停止任务", "循环重头" },
                PlayerPoolExhausted == "loop" ? "循环重头" : "停止任务");
            cmbPlayerPoolExhausted.Enabled = UsePlayerPool;
            cmbPlayerPoolExhausted.Width = 160;
            ...
            string prefix, bool stayConnected, int autoDisconnect)
        {
            ...
            if (chkUsePlayerPool != null) chkUsePlayerPool.Checked = UsePlayerPool;
            if (cmbPlayerPoolExhausted != null)
            {
                cmbPlayerPoolExhausted.SelectedItem = (PlayerPoolExhausted == "loop") ? "循环重头" : "停止任务";
                cmbPlayerPoolExhausted.Enabled = UsePlayerPool;
            }
        }
        ...
        private void SaveAndClose()
        {
            try
            {
                ...
                // ★ 玩家池
                UsePlayerPool = (chkUsePlayerPool != null && chkUsePlayerPool.Checked);
                if (cmbPlayerPoolExhausted != null)
                {
                    PlayerPoolExhausted = (cmbPlayerPoolExhausted.SelectedIndex == 1) ? "loop" : "stop";
                }

                // 字典模式和玩家池互斥
                if (UseNameDict && UsePlayerPool)
                    throw new Exception("「英文名字典」和「玩家池」不能同时启用");
